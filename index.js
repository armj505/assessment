...
function countElectronicsProducts(products) {
  ...
  return electronicsProducts.length;
}
...
function getCheapestProduct(products) {
  // Your code here
  priceArray = [];
  products.forEach((x) => priceArray.push(x.price));
  let cheapest = Math.min(...priceArray);
  return products.find((x) => x.price == cheapest);
}
...
function removeProductById(products, productId) {
  // Your code here
  updateProducts = products.map((x) => {
    if (x.id == productId) {
      products.splice(products.indexOf(x), 1);
    }
  });
  return products;
}
